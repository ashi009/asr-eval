...
export function Layout() {
  ...
  return (
    <div className="flex h-screen bg-background-light dark:bg-slate-900 font-display text-slate-900 dark:text-slate-100 overflow-hidden relative">
      ...
      <aside
        className={`border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex flex-col shrink-0 transition-all duration-300 ease-in-out z-[50]
          ${isOverlay
        ...
        `}
      >
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 w-full">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <AudioLines className="text-primary" />
              <h1 className="font-bold dark:text-white">ASR Eval Pro</h1>
            </div>
            <div className="flex items-center gap-0.5">
              ...
              <ThemeToggle />
              <button onClick={toggleSidebar} className="p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors lg:hidden">
                <span className="sr-only">Close sidebar</span>
                <div className="w-4 h-4 flex flex-col justify-between">
                  <span className="w-full h-0.5 bg-current block origin-center transform rotate-45 translate-y-[6px]"></span>
                  <span className="w-full h-0.5 bg-current block opacity-0"></span>
                  <span className="w-full h-0.5 bg-current block origin-center transform -rotate-45 -translate-y-[6px]"></span>
                </div>
              </button>
            </div>
          ...
          <div className="relative">
            ...
            <input
              className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-md pl-8 py-1.5 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400"
              placeholder="Search..."
              value={search}
        ...
        <div className="flex-1 overflow-y-auto w-full">
          {pendingCases.length > 0 && (
            <div className="relative px-4 pb-4">
              <h3 className="sticky top-0 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md z-20 text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 pt-2 pb-1.5 -mx-4 mb-2">PENDING</h3>
                  ...
                  <li key={c.id} className="min-w-0">
                    <NavLink
                      ...
                      className={({ isActive }) => `block w-full text-left px-0 py-2 rounded text-sm font-mono flex justify-between
                        ${isActive ? 'bg-white dark:bg-slate-800 shadow ring-1 ring-slate-200 dark:ring-slate-700' : 'hover:bg-slate-200/50 dark:hover:bg-slate-800/50 text-slate-600 dark:text-slate-400'}
                      `}
                    >
          ...
          {doneCases.length > 0 && (
            <div className="relative px-4 pb-4">
              <h3 className="sticky top-0 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md z-20 text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 pt-2 pb-1.5 -mx-4 mb-2">DONE</h3>
              <ul className="space-y-1">
                {doneCases.map(c => {
                  return (
                    <li key={c.id}>
                      <NavLink
                        ...
                        className={({ isActive }) => `block w-full text-left px-0 py-2 rounded text-sm font-mono flex justify-between items-center group
                          ${isActive ? 'bg-white dark:bg-slate-800 shadow ring-1 ring-slate-200 dark:ring-slate-700 border-l-2 border-primary' : 'hover:bg-slate-200/50 dark:hover:bg-slate-800/50 text-slate-600 dark:text-slate-400 border-l-2 border-transparent'}
                        `}
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1 px-2">
                          <span className="truncate block">{c.id}</span>
...

      {
        !sidebarOpen && (
          <button
            ...
            className="absolute left-4 top-4 z-50 p-2 bg-white dark:bg-slate-800 shadow-md border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors"
